...
if hash mencoder 2>/dev/null; then
  ...
  mencoder -ovc lavc -lavcopts vcodec=mpeg4:vpass=1:$opt -mf type=png:fps=100 -nosound -o /dev/null mf://\*.png
  mencoder -ovc lavc -lavcopts vcodec=mpeg4:vpass=2:$opt -mf type=png:fps=100 -nosound -o $1 mf://\*.png
